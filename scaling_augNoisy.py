...
def read_points_from_file(filename):
    """Read points from noisy_data files - USE ONLY POSITION 8"""
    points = []
    with open(filename, 'r') as f:
        for line in f:
            line = line.strip()
            if line and line.startswith('r,'):  # Check if it's a data row
                parts = line.split(',')
                if len(parts) >= 9:  # Need at least 9 parts
                    # Use ONLY position 8 (0-indexed)
                    coord_str = parts[8]
                    if '/' in coord_str:
...
if __name__ == "__main__":
    ...
    for class_name in classes:
        input_folder = f"noisy_data/{class_name}"  # Changed to noisy data
        output_folder = f"augmented_noisy_data/{class_name}"  # Changed output folder name
